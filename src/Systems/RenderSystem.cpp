...
// @TODO: Refactor text rendering so that it no longer has to allocate memory at runtime.
...
void queue_text_surface(TTF_Font* font, std::string&& text)
{
	...
	assert(text_surface != nullptr && "Unable to render text surface!");
...
void render_text(SDL_Renderer* renderer)
{
	for (int i = (int)queued_text_surfaces.size()-1; i >= 0; i--)
	{
		...
		assert(text_texture != nullptr && "Unable to create texture from rendered text!");

		SDL_Rect text_dsrect = { (SCREEN_WIDTH / 2) - (text_surface->w / 2), 20+(text_surface->h+24)*i, text_surface->w, text_surface->h };
		
		SDL_RenderCopy(renderer, text_texture, nullptr, &text_dsrect);
...
void render(SDL_Renderer* renderer)
{
	...
	for (unsigned int i = 0; i < entity_alloc->entities.size(); i++)
	{
		...
		SDL_RenderCopyEx(renderer, entity.texture, nullptr, &dest_rect, entity.rotation, nullptr, SDL_FLIP_NONE);
